...
const TableCheckbox = ({tableData, headerData, pagination, onChange, className = "", isScroll = false, defaultValue=[]}) => {
    ...
    const data = tableData?.map((row, index) => {
        ...
        // //         value: headerData[i].render ? headerData[i].render(row[headerData[i].dataIndex], row): row[headerData[i].dataIndex],
        ...
        return (
            <tr key={index}>
                <td style={{width:"5%"}}><input checked={checkedInput(row.id)} type="checkbox" value={row.id} id={row.id} onChange={(event) => checkboxHandle(event)} /></td>
                {pagination ? <td style={{width:"5%"}}>{(pagination.page - 1) * pagination.size + index + 1}</td>:
    ...
    return (
        <div className={`table-container ${className}`}>
            <table className={`table-container__table-checkbox ${isScroll ? "table-container__table-checkbox__scroll": ""}`}>
                <thead>
                <tr>
